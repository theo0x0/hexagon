...
# Hexagon: a next generation internet of finanace
...
Blockchains brought us the same decentralization as the internet did. Early versions consisted only of one asset which can be easily transferred because those transferred happened on dozens of devices, like the internet routs traffic through many networks. Also, it had a functionality of creating new addresses which made possible global access. Unlike the internet blockchains to transfer value needed, state which they could update during each transfer. Another similarity is to access the internet you pay someone to physically connect with you, but with blockchain you pay nodes to include your transaction.  
...
In edge cases when there’s no more new validators or they are leaving, the vault balance becomes near zero or negative. Such a scenario can be addressed in two ways. First, the fees paid for transactions can be used to increase vault balance. Second is a multiplier of each staker can be increased, now it’s simply x2 - the amount you staked is added to the vault. This can be increased by adding to the vault x2.5 or x3; this would be an equivalent of x1.5 or x2, because previously added stake is spent. This is still going to be inflation neutral, because if you withdraw your stake S, from vault will be removed S*2. 
